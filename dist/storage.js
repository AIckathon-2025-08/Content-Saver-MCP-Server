...
export class Storage {
    ...
    async persist() {
        try {
            ...
            throw new Error(`Failed to write storage file at "${STORAGE_FILE}": ${error}. ` +
    ...
    findDuplicateUrl(url) {
        ...
        return this.items.find(item => item.type === 'link' &&
            item.url &&
            this.normalizeUrl(item.url) === normalized) || null;
    }
    /**
     * Save a new item
     */
    ...
    searchItems(filters) {
        ...
        if (filters.tags && filters.tags.length > 0) {
            ...
            results = results.filter(item => item.tags.some(tag => tagSet.has(tag.toLowerCase())));
        }
     ...
     * Get recent items
     */
    getRecentItems(days = 7, limit) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        let results = this.items.filter(item => new Date(item.createdAt) >= cutoffDate);
        // Sort newest to oldest
        results.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    async updateItem(id, updates) {
        ...
        if (updates.url !== undefined && existingItem.type === 'link') {
            ...
            if (newUrl && newUrl !== existingItem.url) {
                ...
                if (duplicate) {
                    throw new Error(`Duplicate URL: a link with this URL already exists (id: "${duplicate.id}"` +
